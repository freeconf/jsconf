...
function reflectChildObject(self: Reflect): node.Node {
    return basic({
        ...
        onChoose: function(sel: node.Selection, choice: meta.Choice): meta.ChoiceCase {
            for (let [_, kase] of choice.cases) {
                for (let d of kase.dataDef) {
        ...
        onField:function(r: node.FieldRequest, hnd: node.ValueHandle): void {
            let prop = Object.getOwnPropertyDescriptor(self.obj, r.meta.ident); 
            if (r.write) {
                if (prop == undefined) {
...
function reflectProp(item: any, ident: string): any {
    let d = Object.getOwnPropertyDescriptor(item, ident);
    if (d != null) {
        return d.value;
    }
    return null;
    ...
    return basic({
        ...
        onNext: function(r: node.ListRequest): ({n: node.Node; key: val.Value[]} | null) {
            ...
            if (r.create) {
                ...
                if (r.row < i.length) {
                    item = i[r.row];
                    let keyVal = reflectProp(item, r.meta.key[0]);
                    key = node.values(r.meta.keyMeta, keyVal);
                }
            }
            if (item != null) {
                let n = reflect({obj:item, onCreate: self.onCreate});
